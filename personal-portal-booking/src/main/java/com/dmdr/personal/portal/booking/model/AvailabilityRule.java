...
public class AvailabilityRule {
	...
	// UTC offset for this rule (e.g., "+00:00", "+05:30", "-05:00"). If null, uses
	// default UTC offset from BookingSettings
	@Column(name = "utc_offset", length = 10)
	private String utcOffset;

	// ruleStatus must be represented as string in DB ("ACTIVE" "INACTIVE"
	// "ARCHIVED")
	// ruleStatus will be automatically move to ARCHIVED after ruleEndInstant.
	// Rename it ti Status
	private RuleStatus ruleStatus;

	// Rule can't be turn ACTIVE if there is another ACTIVE Rule which cross this
	// one.
	// When this.rule is switchig to ACTIVE we will find all ACTIVE rules where
	// that.ruleStartInstant is before this.ruleEndInstant and that.ruleEndInstant
	// is after this.ruleStartInstant
	// We will iterrate over this set of Rules and filter those that.daysOfWeek
	// containsAny this.daysOfWeek
	// Then we filter those where that.vailableStartTime is before
	// this.availableEndTime and that.availableEndTime is after
	// this.availableStartTime
	// If there are no such rules, we can turn this rule to ACTIVE, otherwise we
	// can't turn it to ACTIVE.
