...
- `ServiceEndpointPathsAdminController`
...
- `methods` (**required**, non-empty repeatable HTTP method values, for example `methods=GET&methods=POST`)
- `templatePaths` (**required**, non-empty repeatable endpoint template paths)

### Validation Rules

- `from` must be earlier than or equal to `to` (including `from == to` for a single-day range).
- `methods` values are trimmed, uppercased, de-duplicated, and blank values are removed.
- `templatePaths` values are trimmed, de-duplicated, and blank values are removed.
- `methods` and `templatePaths` must each still contain at least one value after normalization.
...
- `to` (**required**, `YYYY-MM-DD`)
- `methods` (**required**, non-empty repeatable HTTP method values, for example `methods=GET&methods=POST`)
- `templatePaths` (**required**, non-empty repeatable template paths)
...
- Grouping is always `method + templatePath` for this endpoint.
- Result rows are ordered by `method,asc`, then `templatePath,asc`.
...
- Filtering is `createdAt >= from` and `createdAt < to` (`to` is exclusive).
- Maximum allowed range is enforced as `Duration.between(from, to).toDays() <= 31`.
- Invalid range returns HTTP `400 Bad Request`.
